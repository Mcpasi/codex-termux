...
- optional just-in-time permissions pause each command, file edit and terminal input until you allow or deny it: start with `codex --enable just_in_time_approvals --ask-for-approval on-request`. Approvals apply once; local tools keep `CODEX_HOME` private and require a managed sandbox. Embedded apps can use the same [app-server approval flow](./codex-rs/app-server/README.md#just-in-time-permissions).
